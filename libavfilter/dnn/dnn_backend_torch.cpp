                                ...
                                const THRequestItem *request)
{
     ...
     if(th_model->model.func_type == DFT_PROCESS_FRAME){
         *infer_request->output = result.toTensor();
         ...
         if(result.isTuple()){
             ...
             *infer_request->output = calculate_similarity(image_embeddings, text_embeddings, ctx);
         }
     }
     else {
 ...
 static DNNModel *dnn_load_model_with_tokenizer_th(DnnContext *ctx, DNNFunctionType func_type,  const char** labels, int label_count, const char* tokenizer_path, AVFilterContext *filter_ctx){
     ...
     if ((func_type == DFT_ANALYTICS_CLIP || func_type == DFT_ANALYTICS_CLAP) && init_clxp_model(th_model,func_type,labels,label_count,tokenizer_path,filter_ctx) > 0) {
         return NULL;
     }
     #endif
     return &th_model->model;
