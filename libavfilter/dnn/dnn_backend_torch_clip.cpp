...
int get_tokenized_batch(THClipContext *clip_ctx, const char **labels, int label_count, const char *tokenizer_path,
    ...
    if (tokenizer_path) {
        tokenizer = create_tokenizer(tokenizer_path, ctx);
        if (!tokenizer) {
    ...
    // Get tokens using the C tokenizer interface for batch processing
    ret = tokenize_text_batch(tokenizer,
                              labels,
                              label_count,
                              CLXP_EMBEDDING_DIMS,
                              &tokens_array,
    ...
    for (int i = 0; i < label_count; i++) {
        ...
        attention_tensors.push_back(torch::tensor(current_attention, torch::kInt64));
    }
    ...
    for (int i = 0; i < label_count; i++) {
        av_freep(&tokens_array[i]);
    }
    av_freep(&tokens_array);
...
int init_clip_model(THModel *th_model, DNNFunctionType func_type, const char **labels, int label_count,
    ...
    return get_tokenized_batch(th_model->clip_ctx, labels, label_count, tokenizer_path, th_model->ctx,device);
}
...
int prepare_audio_tensor(const THModel *th_model, const THRequestItem *request) {
    ...
    if (batch_size < 1) {
        av_log(ctx, AV_LOG_ERROR, "Not enough samples for processing. Have %d, need at least %d\n", 
    ...
    try {
        ...
        *infer_request->input_tensor = torch::from_blob(audio_data,
