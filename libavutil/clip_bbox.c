...
AVClipBBoxHeader *av_clip_bbox_alloc(uint32_t nb_bboxes, size_t *out_size) {
    size_t size;
    struct ClipBBoxContext {
        AVClipBBoxHeader header;
        AVClipBBox boxes;
    };
    const size_t bboxes_offset = offsetof(struct ClipBBoxContext, boxes);
    const size_t bbox_size = sizeof(AVClipBBox);
    AVClipBBoxHeader *header;
...
AVClipBBoxHeader *av_clip_bbox_create_side_data(AVFrame *frame, uint32_t nb_bboxes) {
    ...
    header = av_clip_bbox_alloc(nb_bboxes, &size);
    if (!header)
        return NULL;
    ...
    if (!av_frame_new_side_data_from_buf(frame, AV_FRAME_DATA_CLIP_BBOXES, buf)) {
        av_buffer_unref(&buf);
        return NULL;
    }
